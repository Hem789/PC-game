...
public class Helicopter : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
       ...
       if(manager.Pause==false)
       {
        ...
        //{
            transform.Rotate(0,.7F*Input.GetAxis("Horizontal"),0);
            ...
            direction=(transform.forward*Input.GetAxis("heliForward")*3/*+transform.forward*1*/)+transform.right*Input.GetAxis("heliRight")*2;
            //}
            //else
            //{
		...
		float rotz = -Input.GetAxis ("heliRight");
		...
		transform.localEulerAngles += new Vector3 (0, Input.GetAxis ("Horizontal") * .5F, 0);
       //transform.localEulerAngles += new Vector3 (0, joystick2.Horizontal * .5F, 0);
        ...
        if(Input.GetAxis("heliForward")!=0 || Input.GetAxis("heliRight")!=0)
