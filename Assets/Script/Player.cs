...
public class Player : MonoBehaviour
{
    ...
    private float time=8,reloadTime,x;
    ...
    void Update()
    {
        ...
        {
        //if(Input.GetKey(KeyCode.N)&& x<=0)
        if(Input.mouseScrollDelta.y!=0 && x<=0 )
        {
            //
            
                manager.weaponchange();
                x=1;
            
        }
        if(manager.zoom==true )
        {
            if(Input.GetKey(KeyCode.Z)&& x<=0)
            {
                manager.FOV();
                x=1;
            }
        ...
        if(Input.GetKey(KeyCode.C) && x<=0)
        {
            sit(); 
            x=1;
        }
        if(Input.GetKey(KeyCode.R) && x<=0)
        {
            reload(); 
         //   x=1;
        }
        ...
        Direction=piv.transform.forward*Input.GetAxis("Vertical")/*joystick.Vertical*/+piv.transform.right*Input.GetAxis("Horizontal");//joystick.Horizontal
        ...
        {
            if(Physics.Raycast(transform.position,new Vector3(0,-1,0),out ground1,0.5F))
            {
